...
#define PLATFORM_X -0.67
#define PLATFORM_Y -0.33
...
int main(int argc, char **argv) {
    ...
    while ((wb_robot_step(TIME_STEP) != -1) && (trial < TRIALS)) {
        ...
        const double *rot_value = wb_supervisor_field_get_sf_rotation(rot_field);
        double distance = sqrt(pow(PLATFORM_X-trans_value[0],2.0)+pow(PLATFORM_Y-trans_value[1],2.0));
          
        // check if the robot found the platform          
        if (distance < 2*PLATFORM_RADIUS) {
            found = true;
        }
